<x-layout.app :title="'PowerForm: ' . ($powerform->name ?? 'Details')">
    <div x-data="{
        ...
        <div x-show="!loading && powerform" class="space-y-6">
            ...
            <div class="flex items-start justify-between">
                <div class="flex-1">
                    <div class="flex items-center space-x-3">
                        <h1 class="text-2xl font-bold text-primary" x-text="powerform?.name"></h1>
            ...
            <x-ui.card>
                <h2 class="text-lg font-semibold text-primary mb-4">Public Access</h2>
                <div class="space-y-4">
                    <!-- Public URL -->
                    <div>
                        <label class="block text-sm font-medium text-primary mb-2">Public URL</label>
                    ...
                    <!-- Embed Code -->
                    <div>
                        <label class="block text-sm font-medium text-primary mb-2">Embed Code</label>
                        <div class="flex items-center space-x-2">
                            <textarea
                                readonly
                                rows="3"
                                class="flex-1 px-3 py-2 border border-border-primary rounded-md bg-bg-secondary text-primary font-mono text-xs"
            ...
            <!-- PowerForm Details -->
            <x-ui.card>
                <h2 class="text-lg font-semibold text-primary mb-4">PowerForm Details</h2>
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <dt class="text-sm font-medium text-text-secondary">PowerForm ID</dt>
                        <dd class="mt-1 text-sm text-primary font-mono" x-text="powerform?.id"></dd>
                    </div>
                    <div>
                        <dt class="text-sm font-medium text-text-secondary">Template</dt>
                        <dd class="mt-1 text-sm text-primary" x-text="powerform?.template_name || 'N/A'"></dd>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-primary" x-text="powerform?.allow_multiple_submissions ? 'Allowed' : 'Not Allowed'"></dd>
                    </div>
                    <div>
                        <dt class="text-sm font-medium text-text-secondary">Created</dt>
                        <dd class="mt-1 text-sm text-primary" x-text="formatDate(powerform?.created_at)"></dd>
                    </div>
                    <div x-show="powerform?.email_subject">
                        <dt class="text-sm font-medium text-text-secondary">Email Subject</dt>
                        <dd class="mt-1 text-sm text-primary" x-text="powerform?.email_subject"></dd>
                    </div>
                </dl>
            </x-ui.card>
            ...
            <x-ui.card>
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-lg font-semibold text-primary">Recent Submissions</h2>
